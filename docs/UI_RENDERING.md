...
> **雙屬性不在建造選單裡**(2026-07-23 改的):改成升級解鎖的選項,在塔面板操作,見下面「加第二屬性」。
...
- 塔面板的升級/加第二屬性按鈕直接 `classList.toggle('cost-unaffordable', poor)`

### 加第二屬性(塔面板)

- **等級還不到 `DUAL_ELEMENT_MIN_LEVEL` 就整顆不顯示**(不是顯示成 disabled)——面板上擺一顆永遠按不下去的按鈕只會讓玩家困惑「這什麼時候能按」,不如等真的能用了再出現
- 已經有第二屬性(定案不能改)、或玩家可用屬性不足 2 種(沒有組合可選)也一樣不顯示
- 點下去跳 `showChoiceModal()` 選要加哪一種(排除主屬性),只列這個玩家自己允許的屬性
- 按鈕用紫色跟一般升級(綠)區分開——它是「換一種塔」不是單純變強

> ⚠️ **塔面板打開時會蓋住固定在畫面下緣的技能列**。`renderTowerPanel()` 切一個 `body.tower-panel-open` class,CSS 靠它把技能列跟技能說明整組往上推。用 body class 而不是 `:has()` 選擇器,不依賴瀏覽器對 `:has()` 的支援。
...
**2026-07-23 修水平置中**:原本 `#gameCanvasWrap` 是 `width:100%` + `max-height:100%`,高度受限時 `aspect-ratio` 會把畫出來的內容縮窄(實測 1280 → 955),但這個 flex item 的 **box 仍然佔著 100% 的寬度**,於是內容整個靠左、右邊留一大塊黑(塔面板一開就特別明顯,因為可用高度變小、寬度縮得更多)。改成讓寬度由高度推算(`height:100%` + `width:auto` + `max-width:100%`),box 本身就是內容的實際寬度,再用 `margin:auto` 讓水平/垂直都置中。
