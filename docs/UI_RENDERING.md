...
結算彈窗附**戰績表**(`renderMatchResultStats()`,各玩家傷害/擊殺/塔數依傷害排序,資料跟記分板同一套 `playerStats`;每場只建一次,innerHTML 沒有點擊被吃的疑慮)。

## 建造虛影(2026-07-24 加的)

lockstep 有 input delay(多人 300ms+ 再加網路往返),點了建造要等一下才看到東西長出來,玩家容易以為沒點到而連點。`submitBuildAction()` 送出建造指令時記下格子(`pendingBuilds`,含 3 秒逾時),GameScene 在該格畫脈動金框;模擬反映(那格長出東西)或逾時(指令被 sim 安全忽略,例如錢不夠)就移除。純顯示,`Date.now()` 用在 UI 層沒有決定性疑慮。

## 音效(2026-07-24 加的,`src/game/sound.ts`)

**全部用 WebAudio 振盪器程序合成,沒有任何音檔資產**(跟免伺服器精神一致,不用煩惱授權/流量)。要點:

- 瀏覽器自動播放政策:`unlockAudio()` 綁在第一次 `pointerdown`(once),之前所有播放呼叫都是安全 no-op
- 觸發靠「跟上一個 tick 的 state 差值」(`playStateSounds()` 的 `soundSnapshot`):擊殺=killSum 增加、建造/拆除=四種建築總數增減、漏怪=生命總和減少、換波=波數增加;攻擊聲另外用 `combatEvents` 有無 + 90ms 節流(不然是機關槍噪音)
- 一次性音效直接在事件點呼叫:勝敗(`showMatchResultOverlay`)、出局、送禮
- 靜音鈕在 `#hudToggles`(「音效:開/關」),偏好存 localStorage(`wuxing-keep:muted`)
- 模擬層完全不知道音效存在,不影響決定性;RESYNC 整份換 state 時差值跳一下可能多響一聲,無害
...
- **開局預設拉近 `DEFAULT_USER_ZOOM`(1.4)、視野置中在地圖中央**(2026-07-24 加的,玩家反映 fit 全圖時內容太小)——`resetCamera()` 每場都重設回這個狀態,不沿用上一場調過的縮放;按「全圖」按鈕隨時回到 1。⚠️ verify-browser 的點擊測試因此都要先按 `#zoomResetBtn` 回 fit,不然用畫布比例算的座標會落錯格子
