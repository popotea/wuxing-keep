...
- `SimulationState.individualLivesMode` 開啟時,`createInitialState()` 把排序後的玩家 id 依 `index % PATH_COUNT` 分組指派給每條路徑(`pathOwners`。~~純顯示用~~ **2026-07-24 起也是塔的攻擊限制依據**,見下;蓋塔位置仍不設限)
- **依人數開線(2026-07-24 加的)**:玩家數少於路徑數時,沒分到負責人的路線「未啟用」——開局 `pathLives` 直接是 0(生怪過濾/全滅判定都吃這個既有欄位,不用加新狀態);`step()` 把生怪**重新分配**到有人負責的路線(不是丟掉,不然波次怪量會縮水變相變簡單);未啟用路線不能緊急補命(補下去會把不生怪的線「啟用」開始湧怪,是害隊操作,no-op)。四線地圖(`quad`)搭配這個機制:2 人開 2 線、4 人開 4 線
- 每條**啟用**路徑各自拿到 `startingLivesPerOwnedPath()`(= `STARTING_LIVES / 啟用線數`)的獨立生命池子(`pathLives`,**平分而不是每條都滿額**,避免總容錯量憑空變兩倍;UI 的血條比例/無傷成就判定也用同一個函式,不要各自重算)
- `step()` 漏怪扣血分岔:開啟時扣 `pathLives[monster.pos.pathId]`(舊的 `lives` 欄位整場凍結不動),關閉時維持原本扣團隊共用的 `lives`
- **某條路徑的生命歸零,那條路徑停止生怪**(`step()` 生怪前先過濾掉 `pathLives[spawn.pathId] <= 0` 的 `SpawnEvent`,現有怪物不會消失),其他路徑完全不受影響繼續進行。⚠️ 這個過濾是**每 tick 動態判斷**的——用緊急補命把該路徑的生命補回 `> 0`,生怪就會恢復(=路徑「復活」,見下面的出局機制;之前文件寫「永久停止」是不精確的)
- **全部路徑都歸零才算 `gameOver`**(`next.pathLives.every((l) => l <= 0)`)
- **個人出局(2026-07-24 加的)**:`isPlayerEliminated()`——某玩家負責的路徑**全部**歸零時,他的指令除了 `emergency_heal`(可以救自己的路徑=買回復活,隊友也能救)跟 `gift_gold`(把用不到的錢支援隊友)以外一律被 `applyCommand` 忽略,變成觀戰狀態。UI 端(main.ts)用同一個函式鎖操作入口(建造選單/塔選單/技能列/跳波按鈕)並顯示 `#eliminatedBanner` 常駐提示;sim 是權威,UI 只是回饋。這是 `step()` 行為變更,新舊版本混連會分岔(APP_VERSION 檢查會擋)。`verify-sim.mjs` 第 [11] 節是回歸測試
...
**路徑守備限制(2026-07-24 加的,使用者的產品決策)**:個人生命模式下**塔只攻擊「塔主人負責路徑」上的怪**(`towers.ts` 的 `canTowerDefendPath()`,`findTarget` 跟 splash 波及都套用)——路徑交叉/相鄰的地圖上,A 的塔會順手把 B 路徑的怪打死,稀釋掉「各守各的路」的意義(實測 crossroads 兩路直接交叉、trident 也有兩個交叉點)。設計細節:

- **無人負責的路徑(owners 為空)任何塔都能打**:玩家數少於路徑數時會出現,不放行的話那條路沒有任何火力管得到,必掉
- 只限「塔的攻擊」:主動技能(隕石/寒冰/戰吼)、陷阱減速**不受限制**——技能是冷卻限制的救命手段(跟它刻意不檢查 moveType 同一個理由),陷阱是減速不是擊殺
- 團隊模式/單人完全不受影響(`step()` 傳 null 給 `tryAttack`,行為跟以前位元級相同)
- 不加任何 state 欄位:`pathOwners` 是開局定案的靜態資料、所有機器一致,checksum 不用動;但這是 `step()` 行為變更,新舊版本混連會分岔(APP_VERSION 檢查會擋)
- UI 端三處提示(蓋塔選單標頭警告「這裡搆不到你的路徑」、塔選單標頭警告「不會開火」、tooltip 說明),不然塔不開火看起來像 bug——`main.ts` 的 `towerWouldBeIdle()` 跟 sim 同一條規則
- `scripts/verify-sim.mjs` 第 [10] 節是回歸測試(不開火/照常開火/團隊模式不受影響/無人負責路徑放行)
