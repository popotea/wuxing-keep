...
**尚未做的延伸(無限模式)**:
- **最佳紀錄/今日最佳/成就系統目前刻意不吃無限模式的結果**——現有系統是繞著「固定 8 波全破」設計的(`{wave, cleared}` 這組欄位),無限模式波次可以遠超過 8,兩種模式數字混在一起比較沒有意義。之後如果要讓無限模式也有紀錄可比,需要另外設計一套獨立的比較基準(例如「最高撐到第幾波」單獨存一個 localStorage key),不是直接共用現有欄位或函式。
- 目前無限模式每波固定 8 隻怪(`ENDLESS_WAVE_MONSTER_COUNT`),沒有讓隻數也隨波次增加——只靠血量/速度變強撐「持續變強」的感覺,之後想再加一層「隻數也增加」的變化也可以。
- 首領波目前固定「每 5 波」且只有 1 種收尾造型(單隻厚血慢速),沒有像固定模式一樣有專屬美術/特殊行為,之後可以考慮讓不同輪次的首領有不同特性(例如某幾輪首領改成群體而不是單體)。
