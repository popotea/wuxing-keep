...
**✅ 雙屬性塔已完成(2026-07-21),2026-07-23 改成升級解鎖**:`towers.ts` 的 `Tower.secondElement`(可選欄位,不存在就是一般單屬性塔),一旦定案就不能改。

> **2026-07-23 的調整**:原本是「建塔時的另一種選項」(建造選單直接有一項「雙屬性塔」),玩家一開局就能蓋。改成要先蓋一般塔、升到 `DUAL_ELEMENT_MIN_LEVEL`(2 級)才能加第二屬性——雙屬性是這套元素系統裡最強的組合(對任何目標最差就是 neutral),開局就有等於把終局手段前置了,少了成長感。現在的節奏是「1 級一般塔 → 2 級可選加第二屬性(不強制)→ 3 級選分岐路線(必選)」,費用是雙屬性造價減單屬性造價的**差額**(蓋塔時已付過單屬性的錢,只補差額)。舊的 `build_dual_tower` 指令已移除,改成 `add_second_element`。
...
**✅ 元素異常狀態已完成(2026-07-23)**:原本列在這裡的「屬性組合異常狀態(DOT)」,實作時改成更完整的版本——不是只有 DOT,而是**每個屬性各自獨佔一種效果**,而且五種效果的「作用面」刻意都不一樣(扣血 / 減速 / 定身 / 增傷 / 位移),不是同一種效果的強弱版本:火=灼燒(DOT)、水=冰緩(減速 40%)、木=纏繞(完全定身,時間刻意很短)、金=破甲(受到傷害 +30%)、土=擊退(瞬間沿路徑往回推,不是持續狀態)。完整數值與設計理由見 `docs/SIMULATION.md` 的「`statuses.ts`」段落。

- **觸發判定不能用 `Math.random()`**:走 `statusRoll(tick, towerId, monsterId)` 純雜湊,每隻怪各自獨立判定(splash 波及的目標不會全部一起中或一起不中);機率隨塔等級成長(18% + 每級 4%)
- **首領有控制抗性**(纏繞/冰緩/擊退時間打 6 折),避免被一排木塔永久定住
- **雙屬性塔只吃主屬性的狀態**:兩種都給的話等於雙屬性塔在戰術效果上也全拿,跟「用基礎數值折扣換取沒有致命對位」的既有平衡精神不一致
- **順手做的架構整理**:新增統一的扣血入口 `dealDamage()`(`towers.ts`),破甲增傷 + 護盾吸收都在裡面,所有傷害來源(塔直擊/splash、灼燒跳傷、隕石技能)都走它——不然很容易變成「有的傷害吃破甲有的不吃」這種很難查的不一致
- **擊退需要 `map.ts` 新增 `retreatAlongPath()`**(`advanceAlongPath` 的反向版),退到起點會夾住不會變負、也不會跑到別條路徑

**尚未做的延伸**:鄰近塔屬性共鳴加成、特殊技能觸發隨機走路方向——這兩項維持原本未拍板的狀態,優先度未定。
...
**✅ 多地圖已完成(2026-07-23)**:`map.ts` 從「單一組寫死的 `PATHS`」改成 `MAP_DEFS` 地圖清單,單人/多人建房都能在開局前選:

| id | 名稱 | 路徑數 | 特色 |
|---|---|---|---|
| `crossroads` | 雙線交會 | 2 | 原本那張(兩條路線在中央交叉一次) |
| `serpent` | 長蛇迴廊 | 1 | 單一條路但來回折返很長,考驗射程覆蓋率 |
| `trident` | 三叉分流 | 3 | 三條路線互不交會,火力必須分散 |

- **所有地圖共用同一個 40x24 尺寸**:畫面縮放、小地圖、裝飾物雜湊都是照這個尺寸算的,不同尺寸要動的地方太多,先統一
- **`PATH_COUNT` 不再是編譯期常數**,改成 `pathCount()`(每張地圖路徑數不同,個人生命模式的生命池子照當前地圖路徑數平分)
...
**✅ 怪物特殊能力已完成(2026-07-23)**:`Monster.ability` 互斥六選一(含 `'none'`),讓「這波該怎麼打」不只看屬性跟移動類型——治療兵(定期回復周圍同伴)、護盾兵(獨立一層護盾,扣完不再生)、分裂怪(死亡分裂成 2 隻,小怪不會再分裂)、急行光環(加速周圍同伴,自己不快)、爆破兵(漏掉扣 3 條命)。

- **固定 8 波刻意「由淺入深」逐波引入**(第 3 波護盾兵 → 第 5 波分裂怪 → 第 7 波治療兵),玩家一次只要應付一個新機制;無限模式從第 4 波開始以 45% 機率抽(一樣走 `waveHash` 決定性雜湊)
- **首領波不帶能力**:牠已經有血量/體型的壓迫感,再疊能力會變成單點無解
- **治療/光環用「兩段式」實作**:先算出這一 tick 要補多少/誰被加速,再一起套用——避免「A 補完 B、B 拿補過的血再去補 C」這種依陣列順序而變的連鎖(雖然仍決定性,但難以推理)
- **分裂放在移除死亡怪物之後**:小怪當 tick 不會被攻擊迴圈打到,下一 tick 才進戰場,不然會有很難推理的同 tick 連鎖

**✅ 玩家主動技能已完成(2026-07-23)**:隕石轟炸(範圍傷害)/寒冰風暴(範圍定身+殘留冰緩)/戰吼(範圍內所有塔攻速大增,不分誰蓋的)。兩段式操作:點技能按鈕進入施放模式 → 點地圖決定圓心。

- **不花金幣,只有冷卻**:金幣已經是建塔/升級的資源,技能再吃金幣會變成「有錢才有技能」,反而讓落後的玩家更沒有翻盤手段。冷卻每個人各自獨立
- **維持「只做正面互助,不做互相陷害」的既有原則**:技能只會傷害怪物、增益我方,沒有任何一個能影響隊友的建築或資源
- **隕石/寒冰刻意不檢查 `moveType`**:技能是救命手段,不該因為這波剛好是空中怪就完全失效(塔已經有移動類型限制了,不再疊一層)
- **踩到一個真實 UX bug 並修掉**:`renderSkillBar()` 第一版用 `innerHTML` 整塊重畫(照抄記分板的做法),但這個函式每 50ms 就被呼叫一次來更新冷卻秒數,按鈕 DOM 不斷被銷毀重建——玩家按下去時如果卡在重建空檔,`mousedown`/`mouseup` 落在不同元素上就不會產生 `click`,**點了沒反應**。Playwright 重試 47 次都點不到才發現(`element was detached from the DOM`)。改成按鈕只建立一次、之後只更新文字/`disabled`/class
