...
// ---------------------------------------------------------------------------
console.log('\n[13] 拆除陷阱/資源建築/圖騰(限本人,退基礎造價一半)');
{
  const placementsMod = await loadModule('src/sim/placements.ts', 'placements2.mjs');
  mapMod.setActiveMap(mapMod.DEFAULT_MAP_ID);

  // 找一個路徑格(蓋陷阱)跟一個非路徑格(蓋資源建築/圖騰)
  let pathSpot = null;
  let freeSpot = null;
  for (let x = 0; x < mapMod.GRID_WIDTH && (!pathSpot || !freeSpot); x++) {
    for (let y = 0; y < mapMod.GRID_HEIGHT && (!pathSpot || !freeSpot); y++) {
      if (mapMod.isOnPath(x, y) && !pathSpot) pathSpot = [x, y];
      if (!mapMod.isOnPath(x, y) && !freeSpot) freeSpot = [x, y];
    }
  }

  let s = sim.createInitialState(19, 100, { p1: ['fire'], p2: ['fire'] }, false, false, mapMod.DEFAULT_MAP_ID);
  s.gold.p1 = 999999;
  s.gold.p2 = 999999;
  s = sim.step(s, 0, [
    { playerId: 'p1', action: { kind: 'build_trap', params: { x: pathSpot[0], y: pathSpot[1] } } },
    { playerId: 'p1', action: { kind: 'build_resource_building', params: { x: freeSpot[0], y: freeSpot[1] } } },
  ]);
  check('陷阱/資源建築有蓋成', s.traps.length === 1 && s.resourceBuildings.length === 1);

  // 別人不能拆
  const trapId = s.traps[0].id;
  const buildingId = s.resourceBuildings[0].id;
  s = sim.step(s, 1, [
    { playerId: 'p2', action: { kind: 'sell_trap', params: { trapId } } },
    { playerId: 'p2', action: { kind: 'sell_resource_building', params: { buildingId } } },
  ]);
  check('拆除限本人(別人拆不掉)', s.traps.length === 1 && s.resourceBuildings.length === 1);

  // 本人拆除,退款正確
  const goldBefore = s.gold.p1;
  s = sim.step(s, 2, [
    { playerId: 'p1', action: { kind: 'sell_trap', params: { trapId } } },
    { playerId: 'p1', action: { kind: 'sell_resource_building', params: { buildingId } } },
  ]);
  const expectedRefund = placementsMod.trapSellValue() + placementsMod.resourceBuildingSellValue();
  check('本人可以拆除', s.traps.length === 0 && s.resourceBuildings.length === 0);
  check('退款金額正確', s.gold.p1 - goldBefore === expectedRefund, `實退 ${s.gold.p1 - goldBefore},預期 ${expectedRefund}`);

  // 圖騰同一套
  s = sim.step(s, 3, [{ playerId: 'p1', action: { kind: 'build_rune_totem', params: { x: freeSpot[0], y: freeSpot[1] } } }]);
  const totemId = s.runeTotems[0]?.id;
  const goldBefore2 = s.gold.p1;
  s = sim.step(s, 4, [{ playerId: 'p1', action: { kind: 'sell_rune_totem', params: { totemId } } }]);
  check(
    '圖騰可拆除且退款正確',
    s.runeTotems.length === 0 && s.gold.p1 - goldBefore2 === placementsMod.runeTotemSellValue(),
  );

  // 拆掉資源建築後,座數名額騰出來可以再蓋
  let s2 = sim.createInitialState(19, 100, { p1: ['fire'] }, false, false, mapMod.DEFAULT_MAP_ID);
  s2.gold.p1 = 999999;
  const spots = [];
  for (let x = 0; x < mapMod.GRID_WIDTH && spots.length < 5; x++) {
    for (let y = 0; y < mapMod.GRID_HEIGHT && spots.length < 5; y++) {
      if (!mapMod.isOnPath(x, y)) spots.push([x, y]);
    }
  }
  const cap = placementsMod.MAX_RESOURCE_BUILDINGS_PER_PLAYER;
  s2 = sim.step(s2, 0, spots.slice(0, cap).map(([x, y]) => ({ playerId: 'p1', action: { kind: 'build_resource_building', params: { x, y } } })));
  s2 = sim.step(s2, 1, [{ playerId: 'p1', action: { kind: 'sell_resource_building', params: { buildingId: s2.resourceBuildings[0].id } } }]);
  s2 = sim.step(s2, 2, [{ playerId: 'p1', action: { kind: 'build_resource_building', params: { x: spots[cap][0], y: spots[cap][1] } } }]);
  check('拆除後座數名額騰出、可以再蓋', s2.resourceBuildings.length === cap);
}
