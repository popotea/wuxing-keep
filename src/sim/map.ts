...
/** 畫面上一次看得到的格數(相機視窗大小);地圖比這個大時,GameScene 用滑鼠移到邊緣平移鏡頭。 */
export const VIEWPORT_TILES_W = 22;
export const VIEWPORT_TILES_H = 14;

/**
 * 多條路徑,每條是一串轉折點(格子座標)。相鄰兩點必須是水平或垂直對齊(不能斜線)。
 * 路徑 0 是主要路線;路徑 1 跟路徑 0 在 (29,14) 交叉一次,提升防守難度(不能只顧一條線)。
 * 40x24 格(比原本 22x14 大不少,配合相機平移做出可探索的地圖)——改格子數記得順手用
 * scripts/ 或暫存腳本驗證交叉點數量/邊界有沒有跑掉。
 */
